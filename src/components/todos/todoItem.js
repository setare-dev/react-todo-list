    ...
    return (
        <div className="flex mb-4 items-center" >
            {editMode ? <> <input className='w-full text-grey-darkest h-11 px-2 border-2 border-blue-600 rounded-md mr-2' onChange={inputEditHandler} value={editText} /><button
                ...
                className="flex-no-shrink p-2 ml-2 border-2 rounded  border-blue-600 text-white bg-blue-600">Edit</button> <button
                    onClick={() => setEditMode(false)}
                    className="flex-no-shrink p-2 ml-2 border-2 rounded  border-gray-400 text-white bg-gray-400">Cancel</button></> :
                <>
